...
func main() {
	...
	// Initialize router
	router := initRouterWithMiddleware(serviceRegistry, metrics, logger)
	...
	gracefulShutdown.RegisterCleanup(func() error {
		...
		logger.Printf("Final metrics: %+v", metricsData)
		return nil
	})
...
func loadConfig(configPath string) (*config.Settings, error) {
	...
	return config.LoadSettings(configPath)
}
...
func initRouterWithMiddleware(serviceRegistry *services.ServiceRegistry, metrics *custommw.Metrics, logger *log.Logger) chi.Router {
	r := chi.NewRouter()
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Route("/etc", func(r chi.Router) {
			...
			r.Get("/", etcHandler.GetETCMeisaiList)
			r.Post("/", etcHandler.CreateETCMeisai)
			r.Get("/{id}", etcHandler.GetETCMeisaiByID)
		...
		// Download endpoints (if implemented)
